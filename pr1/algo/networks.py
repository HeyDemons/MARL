...
class SACActor(nn.Module):
    ...
    def __init__(self, state_size, action_size, hidden_sizes=(64, 64), init_w=3e-3, 
        ...
        self.fc2 = nn.Linear(hidden_sizes[0], hidden_sizes[1])
        self.fc3 = nn.Linear(hidden_sizes[1], action_size )
        self.log_std = nn.Parameter(torch.zeros(action_size))  # Learnable log standard deviation
        self.action_low = action_low
        self.action_high = action_high
        self.scale = (action_high - action_low) / 2.0
        self.bias = (action_high + action_low) / 2.0
    ...
    def forward(self, state):
        ...
        x = F.relu(self.fc2(x))
        mu = self.fc3(x)
        log_std = self.log_std.expand_as(mu)  # Expand log_std to match mu shape
        log_std = torch.clamp(log_std, -20, 2)  # Clamp log_std to avoid numerical issues
        return mu, log_std
    ...
    def sample(self, state, deterministic=False):
        ...
        if deterministic:
            action = torch.tanh(mu)* self.scale + self.bias  # Scale and shift the action
            return action, None, None
        
        dist, base_dist = self._get_dist(mu, log_std)

        action = base_dist.rsample()
        log_prob = base_dist.log_prob(action).sum(-1, keepdim=True)
        log_prob -= (2*(np.log(2) - action - F.softplus(-2*action))).sum(-1, keepdim=True)  # Adjust log_prob for tanh transform
        entropy = base_dist.entropy().sum(-1, keepdim=True) 
        action = torch.tanh(action)* self.scale + self.bias  # Scale and shift the action
