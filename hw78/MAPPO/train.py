...
class Runner_MAPPO_MPE:
    def __init__(self, args, env_name, number, seed):
        ...
        self.args.obs_dim_n = [self.env.observation_spaces[agent].shape[0] for agent in
                               self.env.agents]  # obs dimensions of N agents
        self.args.action_dim_n = [self.env.action_spaces[agent].n for agent in
                                  self.env.agents]  # actions dimensions of N agents
    ...
    def evaluate_policy(self):
        ...
        if self.total_steps % 5e5 == 0:
            print("Saving model at total_steps: {}".format(self.total_steps))
            # Save the model
