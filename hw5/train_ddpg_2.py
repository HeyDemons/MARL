...
def train(num_episodes=1000, max_t=1000):
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        agent.noise.reset()
        episode_reward = 0
        stuck_counter = 0 # 初始化卡住计数器
        
        for t in range(max_t):
            action = agent.act(state)
            next_frame, reward, done, truncated, _ = env.step(action)

            # 如果小车停滞不前（例如驶出赛道或卡住），则给予负奖励
            # CarRacing环境的奖励机制是每帧-0.1，每经过一个瓦片+1000/N。
            # 如果奖励持续为负，说明小车没有前进。
            if reward < 0:
                stuck_counter += 1
            else:
                stuck_counter = 0 # 如果有正奖励，重置计数器
            
            # 如果连续超过一定帧数没有进展，则施加惩罚
            if stuck_counter > 100: # 例如，连续100帧没有访问新瓦片
                reward -= 10  # 施加一个较大的负奖励
                stuck_counter = 0 # 重置计数器，避免连续惩罚
...
if __name__ == "__main__":
    ...
    # 训练 agent
    train(num_episodes=5000, max_t=1000)
