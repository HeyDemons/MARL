...
import tensorboard
from algo import MADDPG, MADDPGApprox, ReplayBuffer, MATD3, MASAC
...
def parse_args():
    ...
    parser.add_argument("--algo", type=str, default="MASAC", choices=["MADDPG", "MADDPGApprox", "MATD3", "MASAC"],
...
def train(args):
    ...
    evaluate(env_evaluate, MAgent, logger, record_gif=args.create_gif, num_eval_episodes=10, global_step=0, algo=args.algo)
    ...
    for global_step in tqdm(range(1, args.total_timesteps + 1), desc="Training"):
        ...
        if args.algo == "MASAC":
            actions_list = MAgent.act(states_list, deterministic=False)
        else:
            actions_list = MAgent.act(states_list, add_noise=True, noise_scale=noise_scale)
        ...
        if global_step > args.warmup_steps and global_step % args.update_every == 0:
            for i in range(len(agents)):
                ...
                if args.algo == "MASAC":
                    critic_loss, actor_loss, alpha_loss, alpha = MAgent.learn(experiences, i)
                    ...
                    critic_loss, actor_loss = MAgent.learn(experiences, i)
                    logger.add_scalar(f'{agents[i]}/critic_loss', critic_loss, global_step)
        ...
        if global_step % args.eval_interval == 0 or global_step == args.total_timesteps:
            MAgent.save(model_path)
            avg_eval_rewards = evaluate(env_evaluate, MAgent, logger,
                    num_eval_episodes=10, record_gif=args.create_gif, global_step=global_step, algo=args.algo)
            np.save(os.path.join(logger.dir_name, "agent_rewards.npy"), agent_rewards)
            score = np.sum(avg_eval_rewards)
            if score > best_score:
                best_score = score
                MAgent.save(best_model_path)
